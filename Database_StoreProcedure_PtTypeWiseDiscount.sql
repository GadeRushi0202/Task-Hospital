...
-- update active and deactive status PtType
CREATE PROCEDURE edit_trn_PtTypeActiveStatus
    @PtTypeId INT,
    @IsActive BIT
AS
BEGIN
...
-- upadate active and deactive status PtTypeWiseDiscount
CREATE PROCEDURE edit_trn_PtTypeWiseDiscountActivePtTypes
    @Id INT,
    @IsActive BIT
AS
BEGIN
    ...
    -- Update the IsActive status for the given Id
    UPDATE mst_PtTypeWiseDiscount
    SET IsActive = @IsActive
    WHERE Id = @Id;
END;
